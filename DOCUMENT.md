...
2. 本地服务层：由 `server.ts` 启动器和 `rust-backend` 原生服务组成，负责媒体库扫描、歌词缓存与代理、静态音频访问。
...
2. 执行 `npm run electron:dev`，脚本会并行拉起 `npm run backend:dev` 和 `npm run dev`。
3. `npm run backend:dev` 通过 `tsx server.ts` 启动 Node 包装层；包装层会先编译并启动 `rust-backend/target/debug/radiflow-backend(.exe)`。
4. Rust 后端监听 `127.0.0.1:3000`，提供 `/api/*` 和 `/music/*`。
5. Vite 开发服务器监听 `127.0.0.1:5173`，并把 `/api`、`/music` 代理到 Rust 后端。
6. Electron 通过 `ELECTRON_START_URL=http://127.0.0.1:5173` 加载前端页面。
7. React 启动后初始化媒体库、播放列表、偏好设置、播放会话。

## 4.2 生产环境（打包）启动流程

1. 执行 `npm run build`，Vite 生成 `dist/`（base: './' 相对路径），esbuild 生成 `dist-electron/server.js`，Cargo 生成 release 版原生后端并复制到 `dist-electron/native/`。
2. 执行 `npx electron-builder --win dir portable nsis`，产物在 `release/`。
3. 打包应用启动时，Electron 主进程调用 `startPackagedLocalServer()`，加载内嵌的 `dist-electron/server.js`。
4. `dist-electron/server.js` 调用 `startServer()`，再启动 `dist-electron/native/radiflow-backend(.exe)`。
5. 原生后端优先绑定稳定端口（见“端口稳定性”章节），产生 `http://127.0.0.1:PORT/`。
6. 主进程窗口加载上述 URL，前端继续通过 `/api/*` 和 `/music/*` 访问本地服务。
...
- 打包版负责调用 `dist-electron/server.js` 并维持稳定端口复用。

### server.ts

- Node 启动包装层。
- 对外仍导出 `startServer()`，供 Electron 主进程在开发态和打包态复用。
- 开发态负责构建并拉起 `rust-backend/target/debug/radiflow-backend(.exe)`；打包态负责启动 `dist-electron/native/` 中的原生可执行文件。

### rust-backend/

- Rust 本地服务源码目录。
- `src/main.rs` 提供媒体扫描、封面缓存、歌词缓存、代理接口与生产态静态资源兜底。

### package.json

- 定义项目脚本和依赖。
- 关键脚本：`backend:dev`、`electron:dev`、`build:renderer`、`build:server`、`build`、`electron:preview`、`lint`。
...
- Vite 配置文件。
- 集成 React 与 Tailwind。
- 开发态把 `/api` 与 `/music` 代理到 `127.0.0.1:3000` 的 Rust 后端。
...
- Electron 侧构建输出目录。
- 包含 `server.js` 包装层和 `native/` 下的 Rust 可执行文件。

### scripts/

- 存放 Electron 开发/预览启动脚本，以及打包时复制 Rust 可执行文件的辅助脚本。
- `quick-start.mjs` 会在缺少 `node_modules/` 时自动执行 `npm install`，然后直接拉起 `npm run electron:dev`。
...
- `useLibrary.ts` 只消费 `/api/music` 的响应，不直接感知后端缓存文件。
- 首次启动和手动刷新都会通过后端接口拉取媒体库；是否复用落盘缓存由 Rust 后端决定。
...
项目当前有三类持久化：渲染进程 localStorage、Electron 的 `userData`、以及所选 music 目录下的缓存文件与歌词数据库。
...
- localStorage 由 origin 隔离。打包版本地服务必须绑定稳定端口（见 4.2 节），否则每次启动都是新 origin，所有设置全部丢失。
...
| `server-port.json` | 上次成功绑定的本地服务端口号，确保 localStorage origin 跨启动一致 |

### music 目录缓存与歌词持久化

- `local/library-cache.json`：媒体库扫描结果缓存。
- `local/cover-cache.json.gz`：封面资源压缩缓存。
- `lyric/lyrics.db`：远端歌词成功结果的 SQLite 缓存。
- `lyric/*.lrc`、`lyric/*.yrc`、`lyric/*.txt`：用户手动放入的同名歌词文件，用作远端无结果时的回退来源。

歌词命中顺序：

1. 先查 `lyric/lyrics.db` 中的远端缓存。
2. 未命中时再请求远端搜索与歌词接口。
3. 远端无结果时，再回退到 `lyric/` 目录下与歌曲同名的手动歌词文件。
4. 仅远端成功获取到的歌词会写入 `lyrics.db`，失败结果不会持久化。

---

## 11. 服务层说明

### server.ts 与 rust-backend 的分工

- `server.ts` 是 Electron 兼容包装层，对外保留 `startServer()` 合约。
- 开发态由 `server.ts` 触发 Cargo 构建并启动 debug 可执行文件。
- 打包态由 `server.ts` 启动 `dist-electron/native/` 下的原生可执行文件。
- `rust-backend/src/main.rs` 持有全部 HTTP API、媒体扫描、歌词缓存与生产态静态资源兜底逻辑。
...
#### GET /api/library/cover/:cover_id
...
- 按歌曲标题、歌手和文件名获取歌词
- 内部会优先命中 SQLite 远端缓存，再回退到手动歌词文件
...
- 仅扫描一层目录，不递归子目录
- 通过内存缓存与 `local/library-cache.json` 复用扫描结果
- 封面资源写入 `local/cover-cache.json.gz`，并通过 `/api/library/cover/:cover_id` 返回给前端

### 歌词实现特点

- 远端歌词缓存持久化在 `lyric/lyrics.db`
- 保留对旧版歌词索引文件的迁移兼容
- 当远端无结果时，会尝试 `lyric/` 目录下的同名手动歌词文件

维护注意：

- 如果未来需要递归扫描，请优先改 `read_library_songs`，并同步检查缓存文件结构是否仍成立。
...
- `window:restart-with-shell-mode` 会调用 `app.relaunch() + app.exit(0)`，进程完全重启。重启后本地服务会优先复用 `server-port.json` 中的端口，保持 localStorage origin 不变。
...
3. 媒体库扫描已引入 `local/library-cache.json` 与 `local/cover-cache.json.gz`，改动扫描逻辑时要同时检查缓存兼容性。
4. 播放列表、播放会话和歌曲 identity 是关联设计，修改其中一个要一起检查其他两个。
5. IPC 与 HTTP API 是两条不同通道：
   - 桌面能力通过 IPC
  - 媒体与歌词数据通过 `server.ts` 包装层启动的本地 HTTP API

这份手册应在核心模块结构变化后同步更新，尤其是以下场景：

- App.tsx 被拆分
- Library.tsx 被拆分
- server.ts 启动合约或 rust-backend 接口结构变化
- 新增持久化机制
- 新增桌面窗口能力
...
2. 打包版未启动本地 HTTP 服务，`/api/*` 接口全部 404。
